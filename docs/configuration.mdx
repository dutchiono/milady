...
```json5
{
  ...
  backend: {},        // Backend runtime selection (legacy-sql vs convex)
...
Milady now supports a top-level `backend` block alongside the legacy `database` block. During the migration window, `database` still controls the legacy SQL implementation, while `backend` selects which runtime path Milady should expose to the app and API.
...
- `backend.kind` controls the migration-era runtime contract exposed by Milady.
- Keep non-secret Convex settings in `backend.convex`. Store `CONVEX_ADMIN_KEY` in Secrets or `env`, not in the visible backend config block.
- `database.provider` remains readable for the legacy SQL path and fallback behavior.
- Convex only becomes the active backend when config is complete and `MILADY_ENABLE_EXPERIMENTAL_CONVEX_RUNTIME=1` is set.
- The initial Convex vertical path is trajectory persistence. Database browsing and raw SQL remain legacy-SQL-only.
