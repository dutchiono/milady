...
function runtimeTodoCircuitKey(runtime: AgentRuntime): string {
  const agentId =
    runtime && typeof runtime === "object" && "agentId" in runtime
      ? String((runtime as { agentId?: unknown }).agentId ?? "")
      : "";
  return agentId.trim() || "unknown-agent";
}
...
function recordTodoDbFailure(
  ...
  const runtimeKey = runtimeTodoCircuitKey(runtime);
  ...
  if (method === "GET" && pathname === "/api/workbench/overview") {
    ...
    if (state.runtime) {
      ...
      try {
        ...
        recordTodoDbFailure(state.runtime, "workbench.summary.todos", err);
  ...
  if (method === "GET" && pathname === "/api/workbench/todos") {
    ...
    if (todoData) {
      try {
        ...
        recordTodoDbFailure(state.runtime, "workbench.todos.list", err);
  ...
  if (method === "POST" && pathname === "/api/workbench/todos") {
    ...
    if (todoData) {
      try {
        ...
        recordTodoDbFailure(state.runtime, "workbench.todos.create", err);
  ...
  if (method === "POST" && todoCompleteMatch) {
    ...
    if (todoData) {
      try {
        ...
        recordTodoDbFailure(state.runtime, "workbench.todos.complete", err);
  ...
  if (todoItemMatch && ["GET", "PUT", "DELETE"].includes(method)) {
    ...
    if (method === "GET" && todoData) {
      try {
        ...
        recordTodoDbFailure(state.runtime, "workbench.todos.get", err);
        // fallback to task-backed path
      }
    }
    ...
    if (method === "DELETE" && todoData) {
      try {
        ...
        recordTodoDbFailure(state.runtime, "workbench.todos.delete", err);
        // fallback to task-backed path
      }
    }
    ...
    if (todoData) {
      try {
        ...
        recordTodoDbFailure(state.runtime, "workbench.todos.update", err);
        // fallback to task-backed path
      }
    }
